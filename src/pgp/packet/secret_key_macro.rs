...
macro_rules! impl_secret_key {
    ($name:ident, $tag:expr, $details:ident) => {
        ...
        pub struct $name {
            pub(crate) details: $crate::pgp::packet::$details,
            pub(crate) secret_params: $crate::pgp::types::SecretParams,
        }
        ...
        impl $name {
            ...
            pub fn from_slice(
                ...
                let (_, details) = $crate::pgp::packet::secret_key_parser::parse(input)?;
                let (version, algorithm, created_at, expiration, public_params, secret_params) =
                    details;
                Ok($name {
                    details: $crate::pgp::packet::$details {
                        packet_version,
                        version,
                        algorithm,
                        created_at,
            ...
            fn repr_from_ciphertext<F>(
                &self,
                pw: F,
                ciphertext: &$crate::pgp::types::EncryptedSecretParams,
            ) -> $crate::pgp::errors::Result<$crate::pgp::types::SecretKeyRepr>
            where
                F: FnOnce() -> String,
            {
            ...
            fn repr_from_plaintext(
                &self,
                plaintext: &$crate::pgp::types::PlainSecretParams,
            ) -> $crate::pgp::errors::Result<$crate::pgp::types::SecretKeyRepr> {
                plaintext.as_ref().as_repr(self.public_params())
            }

            pub fn secret_params(&self) -> &$crate::pgp::types::SecretParams {
                &self.secret_params
            }
        ...
        impl $crate::pgp::types::SecretKeyTrait for $name {
            type PublicKey = $details;

            /// Unlock the raw data in the secret parameters.
            fn unlock<F, G>(&self, pw: F, work: G) -> $crate::pgp::errors::Result<()>
            where
                F: FnOnce() -> String,
                G: FnOnce(&$crate::pgp::types::SecretKeyRepr) -> $crate::pgp::errors::Result<()>,
            {
                use $crate::pgp::types::SecretParams;
            ...
            where
                F: FnOnce() -> String,
            {
                use $crate::pgp::crypto::ECCCurve;
                use $crate::pgp::types::{PublicParams, SecretKeyRepr};

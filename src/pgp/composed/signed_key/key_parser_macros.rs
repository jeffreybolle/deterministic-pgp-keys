...
macro_rules! key_parser {
    ( $key_type:ty, $key_type_parser: ident, $key_tag:expr, $inner_key_type:ty, $( ($subkey_tag:ident, $inner_subkey_type:ty, $subkey_type:ty, $subkey_container:ident) ),* ) => {
        ...
        pub struct $key_type_parser<I: Sized + Iterator<Item = $crate::pgp::packet::Packet>> {
            inner: std::iter::Peekable<I>,
        }

        impl<I: Sized + Iterator<Item = $crate::pgp::packet::Packet>> Iterator for $key_type_parser<I> {
            type Item = $crate::pgp::errors::Result<$key_type>;

            fn next(&mut self) -> Option<Self::Item> {
                use std::convert::TryInto;
                use $crate::pgp::packet::{self, Signature, SignatureType, UserAttribute, UserId};
                use $crate::pgp::types::{KeyVersion, SignedUser, SignedUserAttribute, Tag, KeyTrait};
                ...
                Some(Ok(<$key_type>::new(
                    primary_key,
                    $crate::pgp::composed::signed_key::SignedKeyDetails::new(
        ...
        impl $crate::pgp::composed::Deserializable for $key_type {
            /// Parse a transferable key from packets.
            /// Ref: https://tools.ietf.org/html/rfc4880.html#section-11.1
            fn from_packets<'a>(
                packets: impl Iterator<Item = $crate::pgp::packet::Packet> + 'a,
            ) -> Box<dyn Iterator<Item = $crate::pgp::errors::Result<Self>> + 'a> {
